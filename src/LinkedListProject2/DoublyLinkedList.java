...
import LinkedListProject2.DListNode;
...
public class DoublyLinkedList {
  ...
  public void insertAfter(Object item, DListNode node) {
    // Your solution here.
    if(node != null) {
      DListNode nodeToAdd = newNode(item, node, next(node));
      next(node).prev = nodeToAdd;
      node.next = nodeToAdd;
    }
  }
  ...
  public void insertBefore(Object item, DListNode node) {
    // Your solution here.
    if(node != null) {
      DListNode nodeToAdd = newNode(item, prev(node), node);
      prev(node).next = nodeToAdd;
      node.prev = nodeToAdd;
    }
  }
